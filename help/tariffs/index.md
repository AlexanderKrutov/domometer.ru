...
Тарифы в программе нужны для автоматического расчёта начислений. Тариф всегда привязан к [счётчику]({{ "/help/counters" | absolute_url }}) или [услуге]({{ "/help/services" | absolute_url }}).  

Тариф можно изменить в свойствах счётчика или услуги, тогда указанное значение тарифа будет применено ко всем новым месяцам, добавляемым в [журнал]({{ "/help/journal" | absolute_url }}).  
...
* Находясь на главном экране программы, откройте боковое меню и выберите раздел «[Журнал]({{ "/help/journal" | absolute_url }})». 
...
При открытии раздела «Тарифы» отображается список всех [счётчиков]({{ "/help/counters" | absolute_url }}) и [услуг]({{ "/help/services" | absolute_url }}), добавленных в программе.
...
Каждый элемент списка можно развернуть. В развёрнутом виде отображается следующая информация:
  ...
  * *График тарифов* — по нажатию открывается [график тарифов]({{ "/help/charts-tariffs" | absolute_url }}) выбранного счётчика или услуги
...
Остальные элементы списка — значения тарифов по каждому из месяцев в [Журнале]({{ "/help/readings" | absolute_url }}), отсортированные по времени (последние тарифы в верху списка).
